...
import time, threading, struct, random, configparser, signal, sys
config = configparser.RawConfigParser(converters={'list': lambda x: [i.strip() for i in x.split(',')] if len(x) > 0 else []})
if len(sys.argv) == 2:
...
if use_gstreamer:
    show_bands = config.getint('hppm_proc.py', 'show_bands')
    mute_bands = config.getlist('hppm_proc.py', 'mute_bands')
    if 'None' in mute_bands:
        mute_bands = None
...
def main():
    ...
    showRGBLevels = config.getint('hppm_proc.py', 'showRGBLevels')
    if use_tcp:
    ...
    if use_gstreamer:
        ...
        quit_list['bus'] = bus
        if not use_local:
            quit_list['client'] = setup_osc_client()
    port='NULL'
    if use_ard_int:
        port = start_ard()
    quit_list['port'] = port
    if use_ard_int or use_tcp:
        ...
        if use_nagios:
            ...
            executor, data_t, data_lst, rCount, bCount, gCount = nagios_setup(port)
            quit_list['nagios_executor'] = executor
        while True:
            avg()
            if showRGBLevels:
                print('R:' + str(nwR) + ' G:' +  str(nwG) + ' B:' + str(nwB))
            if use_nagios:
                ...
                if quiet_sets > nagQuietFrames:
                    data_t, rCount, bCount, gCount, offset, frame, data_lst = nagios_sends(executor, data_t,
                         port, offset, rCount, bCount, gCount, frame, data_lst)
                    time.sleep(.2)
                else:
                    main_sends(port)
            else:
...
def nagios_sends(executor, data_t, port, offset, lastrCount, lastbCount, lastgCount, lastFrame, lastdata_lst):
    if data_t.done():
        data_lst, rCount, bCount, gCount = data_t.result()
        # let us know if there are new issues or resolutions
        if bCount > lastbCount:
            flash(2, port)
        elif gCount > lastgCount:
            flash(3, port)
        ...
        offset, newFrame = pushNagFrame(offset, tcp_sock, lastFrame, data_lst, rCount,
            bCount, gCount, port)
        return data_t, rCount, bCount, gCount, offset, newFrame, data_lst
    offset, newFrame = pushNagFrame(offset, tcp_sock, lastFrame, lastdata_lst, lastrCount,
        lastbCount, lastgCount, port)
    return data_t, lastrCount ,lastbCount, lastgCount, offset, lastFrame, lastdata_lst
...
def quit_program(quit_list):
    ...
    if 'pipeline' in quit_list:
        ...
        gi_thread.quit()
    if 'nagios_executor' in quit_list:
        quit_list['nagios_executor'].shutdown(cancel_futures=True)
    if 'osc' in quit_list:
        quit_list['osc'].close()
        quit_list['osct'].join()
    if use_ard_int or use_tcp:
...
def getNagData(hl_params, auth_obj, sl_params, status_url, timeout, port, sleep):
    ...
    try:
        ...
        flash(3, port, e)
    return data_lst, rCount, bCount, gCount
...
def pushNagFrame(offset, tcp_sock, lastFrame, data_lst, rCount, bCount, gCount, port):
    ...
    offset = (offset + 1) % lenData_lst
    return offset, newFrame
...
def playerbin_message(bus, message):
    if message.type == Gst.MessageType.ELEMENT:
        ...
        if struct.get_name() == 'spectrum':
            ...
            if mute_bands is not None:
                for mute_band in mute_bands:
                    m[mute_band] = -60.
            if show_bands:
                for band, level in enumerate(m):
                    if level != -60:
                        print('band:' + str(band) + ' level:' + str(level))
